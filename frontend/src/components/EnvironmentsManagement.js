          ...
          <Dialog open={dialogOpen} onOpenChange={(open) => {
            ...
            <DialogContent data-testid="environment-dialog">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <Button type="submit" className="w-full dark:bg-slate-800" disabled={loading} data-testid="environment-submit-button">
      ...
      <CardContent className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {environments.filter(env => env.name.toLowerCase().includes(searchQuery.toLowerCase())).map((env) => (
            <div 
              key={env.id} 
              className="p-4 border rounded dark:border-slate-600 dark:bg-slate-700/30-lg bg-white dark:bg-slate-800 dark:bg-slate-800 hover:shadow-md transition-shadow"
